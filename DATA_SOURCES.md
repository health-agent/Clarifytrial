...
현재 저장소에는 연구용 원본 데이터셋을 포함하지 않는다. 외부 자료마다 출처, 사용
목적, 이용 조건과 받은 날짜를 기록한다.

| 자료 | 계획한 용도 | 현재 상태 |
|---|---|---|
| ClinicalTrials.gov API v2 | 임상시험 원문과 모집 정보 | 개발 15건과 자연어 평가 본 시험 15건·예비 15건 연결 |
| 팀 공개 ClinicalTrials.gov 시험 모음 | 여러 질환의 넓은 후보 검색 | 1,931건 변환, 모집 중·모집 예정 589건 검색 연결 |
| 공개 시험 조건 기반 합성 평가자료 | 검색부터 질문 뒤 재판정까지 연결 | 10개 질환·공개 시험 50건·조건 202개·합성 환자 50명 |
| 기존 평가와 겹치지 않는 새 시험 자료 | 모델 호출 구조 최종 비교 | 5개 질환·공개 시험 30건·조건 116개·합성 환자 50명 |
...
| 기존 3개 질환 합성 자료 | 기능 회귀와 과거 질문 순서 비교 | 개발 30명·평가 30명 |
...
- SHA256: `95e2551741b82db9e0729e8081af85fe0de559c9cad850b41d849f315bbf889c`
...
- 조건별 정답과 환자-시험 정답: 없음
- 모집 상태별 수: `COMPLETED` 1,221건, `RECRUITING` 430건,
  `NOT_YET_RECRUITING` 138건, `ACTIVE_NOT_RECRUITING` 121건,
  `ENROLLING_BY_INVITATION` 21건
- 현재 검색에 넣는 범위: `RECRUITING`, `NOT_YET_RECRUITING`,
  `ENROLLING_BY_INVITATION` 합계 589건

첨부받은 파일과 고정한 공개 파일의 1,931개 JSON 행과 SHA256이 일치했다. 프로그램은
모집 상태를 먼저 거른 뒤 후보 검색에 넣는다. `prepare-team-trials` 명령은 고정 원본을
내려받아 행 수·중복·해시와 모집 상태를 검사한다. 파일 안에는 수집에 사용한 질환
검색어와 정확한 ClinicalTrials.gov 원자료 시각이 없으므로, 최종 결과를 낼 때는 같은
NCT ID의 최신 모집 상태를 공식 API에서 다시 확인한다.
...
- 복합 조건: 3개 시험의 4묶음
- 합성 환자: 50명
- 생성 기준 시각: 2026-08-25T09:00:00+09:00

최소·최대 나이, 단일 수치 비교, 명확한 상태 조건, 짧은 원문 조건과 원문에서 개수
관계가 분명한 묶음만 구조화했다. 구조화한 202개 조건은 50개 시험의 전체 참가 조건이
아니다. 각 조건에는 공개 시험 번호, 원문 문구, 줄 번호와 구조화 방법을 저장했다.

환자 자료는 모두 합성이다. 가상 환자의 전체 상태를 먼저 정한 뒤 정보 1개·2개·3개 또는
5개를 가리고, 가린 답과 확인 방법을 별도 저장했다. 같은 원본과 설정으로 다시 만든
결과가 저장된 파일과 같은지 `audit-public-protocol-benchmark`로 확인한다.

### 기존 평가와 겹치지 않는 새 시험 자료

- 설정: `configs/independent_new_trial_selection_v1.json`,
  `configs/independent_new_trial_benchmark_v1.json`
- 파생자료: `data/independent_new_trial_benchmark_v1/`
- 원본: 위 팀 공개 ClinicalTrials.gov 시험 모음과 같은 고정 파일
- 평가 시험: 5개 질환·30건, 기존 공개 평가 50건과 중복 없음
- 구조화 조건: 개발 63개·최종 평가 53개
- 합성 환자: 개발 25명·최종 평가 25명
- 정답 고정 시각: 2026-08-25T18:00:00+09:00

각 질환에서 시험 6건을 고른 뒤 앞의 3건을 개발용, 뒤의 3건을 최종 평가용으로
고정했다. 최종 기대 결과는 현재 판정 코드를 부르지 않는 별도 계산기가 구조화 연산자와
합성 수치를 비교해 만들었다. 이 정답은 선택한 객관적 조건 부분에만 해당하며 각 시험의
전체 참가 조건을 뜻하지 않는다.
