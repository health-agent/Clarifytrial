...
> 제공합니다. Fixed-input, 동일예산 FIFO@3와 Oracle Ask-all에 비교해 올바른
> criterion 회복량, 행동 수와 API 비용을 평가합니다. 표적 재평가는 같은 답변을
> 사용한 full rerun과 별도로 비교합니다.
...
5. **평가 계획 20초:** Fixed-input, FIFO@3, Clarify@3, Oracle Ask-all과 targeted/full 비교를 설명한다.
...
3. `docs/research-design-ko.md` — 연구가설, 정답 접근 통제와 공정한 비교 계약
4. `docs/assets/clarifytrial-workflow.png` — 실제 agent 상태 흐름
5. `models.py`와 `rules.py` — 공유 상태와 결정 규칙
6. `tests/` — 102개 자동 검증
