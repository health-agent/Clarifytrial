...
> **현행 기준:** `KOSMI_Abstract_hw_clarity_revised_v2.pdf`<br>
> **확인 시점:** 2026-09-03<br>
> **PDF 생성 시각:** 2026-09-02 23:57<br>
> 아래 내용은 최신 PDF의 초록, 본문, 표, 그림 문구와 참고문헌을 옮긴 내부 기준 원고다.
> 제출 양식의 실제 배치와 줄바꿈은 PDF를 따른다. 현재 PDF는 3쪽이며 참고문헌 4~6번이
> 3쪽으로 넘어가 있어 최종 2쪽 조판은 아직 남아 있다.
...
| 평가 | 자료 | 확인한 내용 |
|---|---|---|
| 미정 시험 정리 | 공개 시험 15건·조건 80개·평가 환자 20명 | 확인 0~3회에서 정보 선택 방법 비교 |
| 연결 구조 | 9종 × 200개 = 1,800개 | 직접 공유·분리·사슬형 구조에서 선택 방법 비교 |
| 환자 제약 | 합성 환자 20명·짝지은 설정 80개 | 모든 경로 허용과 환자 제약 적용 전후 비교 |

환자마다 서로 다른 정보 가림 2개를 만들고, 각 가림에 시험 5건을 연결하였다. 프로그램
기본 한도인 3회를 주 평가로 두고, 0~2회는 확인 횟수에 따른 변화를 보기 위해 함께
비교하였다. 숨긴 정보 5개를 대상으로, 가능한 120가지 확인 순서의 평균을 기준으로 영향
우선 기본 규칙, 개발용 합성 환자의 답변 분포로 다음 확인의 기대효과를 계산하는 방법,
남은 질문의 조합을 함께 계산하는 방법 등 다섯 가지 선택 방법을 확인 0~3회에서
비교하였다. 답변분포 기반 선택에 필요한 분포는 평가 환자와 분리한 개발용 합성 환자
10명에서 구하였다. 95% 구간은 질환별 환자 구성을 유지한 채 환자 단위로 5,000회
재표집해 계산하였다.

## Ⅲ. 연구 결과

확인 3회 후 초기 미정 120건 중 88건이 적격 또는 부적격으로 정리됐고, 질문 3개를
무작위 순서로 확인했을 때의 평균 상태 일치율은 78.27%였고, 영향이 큰 정보부터
확인했을 때는 84.0%였다.
...
| 평가 | 비교 기준 | 결과 |
|---|---|---|
| 미정 시험 정리, 확인 3회 | 처음 미정 120건 | 적격 13/30건·부적격 75/90건, 총 88/120건(73.3%) |
| 영향 우선 방식 | 가능한 120개 확인 순서에서의 평균 | 84.0% 대 78.27%, +5.73%p(95% 구간 +3.04~+8.55%p) |
| 답변분포 기반 선택 | 영향 우선 방식 | 86.0% 대 84.0%, 차이 +2.00%p(95% 구간 -2.00~+6.50%p) |
| 연결 구조 | 구조별 비교 | 직접 공유: 영향 우선이 무작위보다 +43.28%p<br>완전 분리: 영향 우선과 무작위의 차이 0.00%p<br>사슬형: 조합 탐색이 영향 우선보다 +3.22~4.27%p |
| 환자 제약 | 모든 경로 허용과 환자 제약 적용 | 새 검사 21→0회·추가 방문 56→0회, 미정 시험 0.88→1.24건 |
...
환자에게 허용되지 않은 검사와 방문을 먼저 제외하자 해당 제안은 사라졌지만 일부 시험은
미정으로 남았다. ClarifyTrial은 이 경우 결론을 강제로 내리지 않고 확인 대기로 남긴다.
향후 실제 환자 자료에서 선별 결과와 모집 과정에 미치는 영향을 검증할 필요가 있다.

## Ⅴ. 결론

ClarifyTrial은 정보가 부족한 시험을 버리지 않고, 제한된 횟수 안에서 여러 미정 시험을
함께 정리할 수 있는 정보를 고른 뒤 환자가 이용할 수 있는 경로로 확인한다. 이번
평가에서 확인 3회 뒤 초기 미정 120건 중 88건이 적격 또는 부적격으로 정리됐다. 하나의
정보가 여러 시험에 직접 연결되면 영향 우선 방식이, 여러 단계가 이어진 사슬형
구조에서는 조합 탐색이 유리했다. 환자 제약을 지키면 실행할 수 없는 검사와 방문은 피할
수 있지만 일부 시험은 미정으로 남았다.
