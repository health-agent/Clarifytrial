...
평가는 세 질문으로 나눴다. 첫째, 확인 횟수가 제한될 때 어떤 정보를 먼저 물어야 하는가.
둘째, 질문 순서의 효과가 시험 간 정보 공유 방식에 따라 달라지는가. 셋째, 환자가 새
검사나 추가 방문을 원하지 않을 때 확인 방법과 남는 미정 시험이 어떻게 달라지는가. 전체
정보를 제공했을 때 나온 후보 유지 여부와 확인 상태를 정답으로 삼고, 두 값이 모두 같을
때 상태가 일치했다고 계산했다. 표 1에 각 평가의 자료와 비교 조건을 정리했다.

**표 1. 평가 설계**

| 실험 | 규모 | 비교 |
|---|---|---|
| 질문 순서 | 시험 15건·조건 80개·합성 환자 20명 | 확인 0~3회 |
| 시험 간 정보 공유 | 연결 형태 9종·1,800개 | 공유·분리·사슬 |
| 환자 부담 | 합성 환자 20명·80개 설정 | 검사·방문 제한 전후 |

질문 순서 평가에서는 환자마다 서로 다른 정보 가림 2개를 만들고, 각 가림에 시험 5건을
연결하였다. 숨긴 정보 5개를 확인 0~3회에서 비교하고, 3회를 주 평가로 삼았다. 영향 우선
방식은 연결된 미정 시험 수를 먼저, 조건 수를 다음으로 비교하며, 둘이 같으면 환자 부담이
적은 정보를 골랐다. 답 분포 기반 방식은 개발용 합성 환자에서 각 답이 나온 비율을
이용해, 다음 질문 뒤 정리될 시험 수가 가장 클 것으로 예상되는 정보를 골랐다. 조합
탐색은 남은 확인 횟수 안에서 가능한 정보 조합을 모두 따져, 가장 많은 시험을 정리하는
순서를 골랐다. 무작위 기준은 숨긴 정보 5개의 120가지 순서를 모두 실행한 평균이다.

시험 간 정보 공유 평가에서는 한 정보가 여러 시험에 함께 쓰이는 직접 공유, 시험마다
필요한 정보가 다른 완전 분리, 한 답이 다음 질문의 필요성을 바꾸는 사슬형 구조를 포함한
9가지 연결 형태에서 질문 선택 방법을 비교했다. 환자 부담 평가에서는 같은 사례를 두 번
실행해, 모든 확인 방법을 허용한 경우와 새 검사·추가 방문을 제한한 경우의 선택 경로와
남은 미정 시험 수를 비교했다.
