...
> **확인 시점:** 2026-09-03<br>
> **HWP 본문·표 수정 시각:** 2026-09-03 02:27<br>
...
먼저 ClarifyTrial의 실행 흐름을 설명하고, 이어 질문 순서, 정보 연결 구조와 환자
부담을 평가하였다.
...
세가지 질문을 평가했다. 1)확인 횟수가 제한될 때 어떤 정보를 먼저 질문하는지 2) 질문
순서의 효과가 정보 연결 구조에 따라 달라지는지 3) 환자가 새 검사나 추가 방문을 원하지
않을 때 확인 방법과 남는 미정 시험이 어떻게 달라지는지. 전체 정보를
제공했을 때 나온 후보 유지 여부와 확인 상태를 정답으로 삼고, 두 값이 모두 같을 때 상태가
일치했다고 계산했다. 세 평가에서 바꾼 조건과 비교 대상을 표 1에 정리했다.

**표 1. 평가 설계**

| 실험 | 규모 | 비교 |
|---|---|---|
| 질문 순서 | 시험 15건·조건 80개·합성 환자 20명 | 가능한 순서 평균·영향 우선·답 분포·조합 탐색 |
| 정보 연결 구조 | 연결 형태 9종·1,800개 | 확인 2·3회<br>단계별 무작위·영향 우선·조합 탐색 |
| 환자 부담 | 합성 환자 20명·80개 설정 | 모든 경로 허용 / 검사·방문 제한 |

질문 순서 평가에서는 같은 환자·시험·가린 정보와 확인 횟수를 두고, 다음 정보를 고르는
방법만 바꿨다. ClarifyTrial의 기본 규칙인 영향 우선은 가장 많은 미정 시험에 연결된
정보를 먼저 고른다. 이 규칙이 우연한 순서보다 나은지, 답의 가능성이나 남은 질문 전체를
계산하면 더 나아지는지를 비교했다. 답 분포 기반은 개발용 합성 환자의 답 비율로 다음
질문 뒤 정리될 시험 수를 예상하고, 조합 탐색은 남은 확인 횟수에서 가능한 정보 조합을
함께 계산한다. 기준값은 가린 정보 5개의 가능한 120가지 순서를 모두 실행한 평균이다.
환자마다 정보 가림 2개와 시험 5건을 적용하고, 확인은 0회부터 3회까지 비교하되 3회를 주
평가로 삼았다.

정보 연결 구조 평가는 선택 방법의 차이가 어떤 구조에서 커지는지 확인하는 실험이다. 한
정보가 여러 시험에 쓰이는 직접 공유, 시험마다 필요한 정보가 다른 완전 분리, 한 답이
다음 질문의 필요성을 바꾸는 사슬형 등 9가지 연결 형태에서 확인을 2회와 3회 허용하고,
단계별 무작위·영향 우선·조합 탐색을 같은 조건으로 비교했다. 환자 부담 평가는 같은
사례에서 모든 확인 방법을 허용한 경우와 새 검사·추가 방문을 제한한 경우를 비교해, 선택
경로와 남은 미정 시험 수가 어떻게 바뀌는지 확인했다.

## Ⅲ. 연구 결과

확인 3회 후 초기 미정 120건 중 88건이 적격 또는 부적격으로 정리됐다. 영향 우선 방식의
상태 일치율은 가능한 순서 평균보다 5.73%p 높았다. 정보 연결 구조에서는 확인 2·3회
모두 직접 공유에서 영향 우선이 단계별 무작위보다 높았고, 완전 분리에서는 차이가
없었다. 사슬형에서는 조합 탐색이 영향 우선보다 높았다.
...
|  | 부적격 | 75/90건 |
|  | 확정 비율 | 88/120건 (73.3%) |
| 정보 선택 (전체 200건) | 가능한 순서 평균 | 78.27% |
|  | 영향 우선 | 84.0% |
|  | 답 분포 기반 | 86.0% |
| 연결 구조 (1,800개) | 직접 공유¹ | +19.03~+30.09%p |
|  | 완전 분리¹ | 0.00%p |
|  | 사슬형² | +3.22~4.27%p |
...
¹ 영향 우선 대 단계별 무작위. ² 조합 탐색 대 영향 우선. (확인 2·3회·두 답 분포 범위)

## Ⅳ. 고찰

확인 2·3회에서 정보 연결 구조에 따라 세 방법의 차이가 달랐다. 직접 공유 구조에서는
영향 우선과 조합 탐색이 같았고 모두 단계별 무작위보다 높았다. 완전 분리에서는 차이가
없었다. 사슬형에서는 조합 탐색이 영향 우선보다 높아, 여러 질문을 함께 계산할 필요가
있었다.
