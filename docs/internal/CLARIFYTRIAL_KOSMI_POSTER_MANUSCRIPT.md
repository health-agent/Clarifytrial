...
먼저 ClarifyTrial의 실행 흐름을 설명하고, 이어 질문 순서, 시험 간 정보 공유와 환자
부담을 평가하였다.
...
세가지 질문을 평가했다. 1)확인 횟수가 제한될 때 어떤 정보를 먼저 물어야 하는가. 2)
질문 순서의 효과가 시험 간 정보 공유 방식에 따라 달라지는가. 3) 환자가 새 검사나 추가
방문을 원하지 않을 때 확인 방법과 남는 미정 시험이 어떻게 달라지는가. 전체 정보를
...
| 시험 간 정보 공유 | 연결 형태 9종·1,800개 | 영향 우선 대 무작위 / 조합 탐색 대 영향 우선 |
| 환자 부담 | 합성 환자 20명·80개 설정 | 모든 경로 허용 대 검사·방문 제한 |
...
시험 간 정보 공유 평가는 위 선택 방법의 차이가 언제 커지는지 확인하기 위한 실험이다. 한
정보가 여러 시험에 함께 쓰이는 직접 공유, 시험마다 필요한 정보가 다른 완전 분리, 한
답이 다음 질문의 필요성을 바꾸는 사슬형을 포함한 9가지 연결 형태를 만들었다. 확인
1회에서는 영향 우선과 무작위 선택을, 확인 2~3회에서는 조합 탐색과 영향 우선을
비교했다. 환자 부담 평가는 같은 사례에서 모든 확인 방법을 허용한 경우와 새 검사·추가
방문을 제한한 경우를 비교해, 선택 경로와 남은 미정 시험 수가 어떻게 바뀌는지 확인했다.
