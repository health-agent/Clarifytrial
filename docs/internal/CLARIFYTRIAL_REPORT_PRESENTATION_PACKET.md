...
정답과 88.8% 일치했다. 따라서
“ClarifyTrial이 TrialGPT보다 조건 판단을 더 잘한다”는 주장은 하지 않는다. 한 번 더
검토하거나 인터넷을 검색하게 해도 맞힌 수가 늘지 않아 해당 기능은 주 구조에서 뺐다.
...
개발 환경을 설치한 뒤 다음 명령을 실행한다. `heldout`은 개발에 사용하지 않은 평가용
환자만 고르는 옵션이다. 외부 모델과 인터넷 검색은 사용하지 않는다.
