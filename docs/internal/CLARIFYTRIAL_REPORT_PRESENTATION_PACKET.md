...
합성 환자에서 가려 두지 않은 전체 정보를 넣어 각 시험의 `후보 유지`, `참가 조건 확인`,
`제외` 판단을 먼저 계산하고 비교 기준으로 사용했다.
...
가장 많은 시험 판단을 끝낼 정보를 매번 계산한 방법은 환자 30명 중 15명에서, 입력
파일에 적힌 순서대로 확인한 방법보다 더 많은 시험의 판단을 끝냈다. 나머지 15명에서는
같았고 더 적게 끝낸 환자는 없었다. 다섯 번까지 확인하면 두 방법 모두 모든 정보를
알 때의 판단에 도달했지만, 도달할 때까지 확인한 정보는 각각 환자당 평균 3.63개와
4.53개였다.

같은 30명에서 핵심 값 다섯 개를 처음 입력에서 완전히 지워도 세 번 확인한 뒤,
가장 많은 시험 판단을 끝낼 정보를 매번 계산한 방법은 88.7%, 처음 빠진 정보 목록의
앞 세 항목을 확인한 방법은 75.3%였다. 전자는 최종 판단에 필요했던 정보를 모두
포함했다. 총 88개 정보를 확인한 뒤 그중 어떤 시험의 판단도 더 바꾸지 못한 정보는
9개였다. 이 필요 여부는 실행이 끝난 뒤에만 계산했다.

검색·조건 판단·질문·답변·재판정을 같은 여러 시험 실행기에 연결한 별도 평가는 30명에서
추가 정보를 확인하지 않은 경우 42%, 처음 빠진 정보 목록의 앞 세 항목을 확인한 경우
66%, 남은 세 번 안에 가장 많은 시험 판단을 끝낼 정보를 매번 계산한 경우 77%였다.
세 번째 방법은 두 번째 방법보다 16명에서 좋고 12명에서 같았으며 2명에서 낮았다.
앞의 75.3%와 88.7%는 정보 선택 규칙만 본 결과이고, 66%와 77%는 검색부터 재판정까지
전체 프로그램을 연결한 결과다.
...
보여 준다. 모든 상황을 합쳤을 때 모든 환자 정보를 알 때와 같은 판단에 도달한 시험
비율은 84.0%에서 79.2%로 낮아졌으므로 환자
환자의 이동·비용·시간 제한을 반영한 방법이 언제나 더 높은 점수를 내는 것은 아니다.
대신 실제 이용 제약이 있는
상황에서는 이용 불가능한 방법을 피하면서 81.0%에서 88.5%로 높아졌다.
...
처음에는 시험 2개의 판단만 모든 정보를 알 때의 판단과 같았다. 처음 빠진 정보 목록의
...
새 합성 환자 30명·시험 판단 150건에서 확인 기회가 세 번일 때, 가장 많은 시험 판단을
끝낼 정보를 매번 계산한 방법은 시험의 89%를 모든 정보를 알 때의 판단과 같게 만들었다.
처음 빠진 정보 목록의 앞 세 항목을 확인한 방법은 75%였다. 이동·비용 부담이 큰 합성
환자에서는 사용할 수 없는 방법을 제외하고도 모든 정보를 알 때의 판단에 도달한 시험이
81.0%에서 88.5%로 늘었다. 환자가 새 검사나 추가 방문을 피해야 한다고 입력했는데도
그런 방법을 선택한 횟수는 65회에서 0회로 줄었다. 검색부터 재판정까지 전체 프로그램을
연결한 별도 평가에서는 두 정보 선택 방법이 각각 66%와 77%였다.
...
5. **대표 사례와 범위:** 2/5에서 5/5로 바뀐 순서를 보여 주고, 공개 조건과 합성
   환자를 쓴 예비평가라는 범위를 밝힌다.
...
| 세 번 확인했을 때 입력 목록 순서 방식은 75%, 가장 많은 시험 판단을 끝낼 정보를 계산한 방식은 89%였다 | 의사와 같은 수준으로 판단한다 |
| 입력된 환자 부담에 따라 확인 방법을 바꿨다 | 실제 환자의 비용과 시간을 줄였다 |
| 관련 시험 검색 결과가 TrialGPT 공개값과 거의 같았다 | 검색부터 최종 추천까지 임상 검증을 마쳤다 |

## 자동 보고서 만들기

다음 명령은 저장된 질문 정책, 환자 부담, 전체 흐름과 검색 결과를 읽어 보고서용 표,
CSV, SVG와 Markdown을 한 폴더에 만든다. 숫자는 입력 결과 파일에서 자동으로 읽는다.
...
범위를 더 넓힐 때는 다른 질환·더 큰 시험 모음과 여러 모델 설정에서 같은 평가를 다시
실행하면 된다.
