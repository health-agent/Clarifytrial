...
질문 없음 42%, 고정 순서 66%, 현재 방식 77%였다. 현재 방식은 고정 순서보다 16명에서
좋고 12명에서 같았으며 2명에서 낮았다. 앞의 75.3%와 88.7%는 질문 순서만 본 결과이고,
66%와 77%는 전체 프로그램 연결까지 본 결과다.
...
보여 준다. 모든 상황을 합친 최종 상태 회복은 84.0%에서 79.2%로 낮아졌으므로 환자
맞춤 방식이 언제나 더 높은 점수를 내는 것은 아니다. 대신 실제 이용 제약이 있는
...
환자에게 이를 제안한 횟수는 65회에서 0회로 줄었다. 전체 실행기를 통한 별도 평가에서는
고정 순서 66%, 현재 방식 77%였다.
