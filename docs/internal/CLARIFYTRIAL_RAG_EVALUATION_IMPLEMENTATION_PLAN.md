...
대표 출력의 안전성과 이해 가능성을 확인하는 일이다. 자연어 표현 다양성은 규격화된
JSON 앞단의 선택 기능이므로 현행 주 평가에 포함하지 않는다.
...
3. 실제로 이용할 수 있는 방법만 사용한 최종 판단 비율이 현재 단순 규칙보다 2%p
   넘게 낮지 않음
...
안전성을 확인한 결과가 아니다. 규격화된 JSON 질문 정책과 환자 부담 경로는 구현과
...
10. 범위 제외 — 자연어 표현 반복은 규격화된 JSON을 기본 입력으로 정하면서 주 평가에서 뺐다.
