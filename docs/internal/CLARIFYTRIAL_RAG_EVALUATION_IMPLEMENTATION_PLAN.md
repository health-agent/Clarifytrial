...
상태: 자연어 입력부터 후보 검색·질문·재판정까지 Sol 한 사례 연결 완료, 자연어 정확도 평가 전
정리일: 2026-08-22
...
일반 환자 실행에서는 단순 동적 규칙이 사실과 확인 경로를 고르고, LLM은 조건 판단과
질문·확인 요청문을 담당한다. 후보 검색부터 실행기까지의 코드 연결은 끝났다. 다음
검사는 공개 원문과 환자 기록의 자연어 구조화 정확도와 잘못된 인용 거부율을 외부
모델 소표본에서 측정하는 일이다. 전체 실행 토큰 기록 형식은 한 사례에서 확인했다.
...
- 구조화 입력의 질문 정책 결과와 자연어 전체 실행·임상 성능을 구분한다. 현재
  자연어 전체 연결은 외부 모델 한 사례만 실행했으므로 정확도와 임상 성능은 `미측정`으로 표시한다.
...
현재 만든 30명은 구조화된 값과 짧은 합성 근거 문장을 함께 가진다. 짧은 진료기록과
문장형 기록을 반씩 만드는 표현 다양화, 원문에 없는 사실이 추가되지 않았는지 검사하는
작업은 자연어 구조화 개발 단계에서 진행한다.
...
| ClarifyTrial | 여러 시험에 미치는 영향과 환자 부담을 보고 다음 사실과 경로 선택 |
...
이 결과는 자료 생성 규칙과 채점 코드가 맞물리는지 확인한 값이다. 비교 시스템은 아직
실행하지 않았다. 두 사람의 조건 검토 전에는 고정 정답이나 성능 자료로 부르지 않는다.
