...
상태: 합성 자연어 기록 해석과 질문 정책 평가 완료, 사람 검토·실제 기록 평가는 전
정리일: 2026-08-22
...
- 모든 비교 구조에 같은 TrialGPT 수준 후보 RAG 제공
...
- 완료 — 한 부족 사실에 기존 기록, 환자 답변, 기존 결과, 새 검사와 의료진 판단 경로 연결
- 완료 — 대기, 방문, 비용, 신체·심리 부담, 의료 위험과 치료 방해 분리
- 완료 — 새 검사·침습 절차·치료 변경의 사람 승인 경계
- 완료 — 기존 30명에 부담 조건을 짝으로 붙인 360개 상황·1,800개 정책 실행
...
- 완료 — 후보 유지와 현재 확인 분리
- 완료 — 여러 시험의 영향과 환자 부담을 함께 보는 확인 경로 선택
- 일부 완료 — 특정 조건 근거만 바뀌면 관련 조건만 재판정. 후보 검색을 바꾸는 정보의 재검색은 아직 전
...
- 완료 — TrialGPT 실행용 후보 검색부터 질문·답변·재판정까지 한 명령으로 연결
- 완료(연결 확인) — Sol `medium` 합성 사례 한 건의 질문·공식 결과 확인·재판정
- 완료(합성 예비평가) — 자연어 기록 해석과 질문 없음·고정 순서·ClarifyTrial 짝 비교
- 완료 — 남은 질문 횟수 안의 사실 조합을 전부 계산하는 v3를 일반 환자 실행에 연결

답 확률과 모든 답변 가지를 가정하는 과거 전수 정책은 비교용으로 남긴다. 새 v3는
숨은 답을 보지 않고 현재 미해결 사실의 작은 조합만 정확히 계산한다. 일반 실행에서는
v3가 사실을 고르고 환자 부담 규칙이 경로를 고른다. LLM은 기록 정리와 질문·확인
요청문을 담당하며 사실·경로 선택을 바꾸지 못한다. 다음 외부 검증은 사람 검토 조건과
다양한 기록 표현에서 같은 결과가 유지되는지 확인하는 일이다.
...
- 구조화 입력의 질문 정책 결과와 자연어 전체 실행·임상 성능을 구분한다. 합성 자연어
  기록과 고정 후보 질문 흐름은 평가했지만 실제 진료기록과 임상 성능은 `미측정`으로 표시한다.
...
4. 완료 — 영향 우선, 고정 경로 비용, 추가 부담 최소, 환자 맞춤, 모두 확인 방식 구현
...
3. 새 검사를 금지하지 않은 환자는 전체정보 기준 회복률, 새 검사를 거부한 환자는 허용 경로 기준
   회복률이 현재 단순 규칙보다 2%p 넘게 낮지 않음
...
전체정보 회복률도 함께 보고한다. 환자가 새 검사를 명시적으로 거부했는데 금지된
검사까지 시행한 결과만 정답으로 두면 한도를 지킨 방식이 자동으로 불리해진다. 따라서
이 경우에는 전체정보 결과와 환자가 허용한 경로만 모두 사용했을 때의 결과를 나란히
둔다. 새 검사를 금지하지 않은 상황은 기존처럼 전체정보 결과를 기준으로 삼는다.
...
2026-08-21에 공개 시험 15건·조건 80개·합성 환자 30명을 사용했다. 환자마다 마스크
2개, 환자 부담 상황 3개와 자료 가용성 2개를 붙여 360개 상황을 만들고 다섯 방식을
적용해 1,800개 정책 실행을 마쳤다. 외부 모델 호출과 모델 토큰 사용은 0이다.

고정 평가 20명에서는 다음 결과가 나왔다.

- 새 검사를 금지하지 않은 상황의 전체정보 회복: 환자 맞춤 83.75%, 고정 경로 비용 84.00%
- 이동·비용 제약 환자의 허용 경로 회복: 환자 맞춤 89.0%, 고정 경로 비용 81.0%
- 이동·비용 제약 환자의 새 검사·추가 방문: 환자 맞춤 0건, 고정 방식 65건
- 시간 긴급 환자의 평균 누적 대기: 환자 맞춤 54.83시간, 고정 방식 62.03시간
...
안전성을 확인한 결과가 아니다. 자연어 전체 실행의 코드 연결과 Sol 한 사례 확인은
끝났으며, 현재 이어서 할 일은 외부 모델 소표본에서 구조화 정확도를 재는 것이다.
...
- 후보 검색을 빼고 후보 유지, 현재 확인, 다음 정보와 재판정만 비교한다.
- 같은 환자와 같은 후보를 모든 방식에 사용한다.

#### 3단계: 후보 검색 포함

- 2단계가 끝난 뒤 새 시험이 들어 있는 고정 ClinicalTrials.gov 말뭉치에 BM25·MedCPT
  결합 검색을 붙인다.
...
환자의 숨은 답, 질문 순서와 질문 뒤 재판정 정답이 없으므로 대화 성능의 정답으로 쓰지
...
7. 개발 환자 10명의 두 근거 상태에서 전체 연결을 검사한다. 오류가 없을 때만 고정
   평가 환자 20명의 두 근거 상태를 연다.
8. 고정 후보 60회에서 네 방식을 비교한다.
9. 후보 검색 포함 평가는 마지막에 한 번 실행한다.
10. 대표 어려운 환자 12명만 한 번 반복해 모델 출력 변동을 기록한다.
...
ClarifyTrial이 더 낫다고 말하려면 다음을 모두 만족해야 한다.

- 강한 한 번 판정보다 잘못된 후보 제거 또는 너무 이른 확인이 줄어듦
- 고정 순서 질문보다 행동 3회 뒤 전체 정보 결과 회복이 낮지 않음
- 구조화 정답 입력보다 자연어 입력 결과가 얼마나 떨어지는지 별도 공개
...
합친 이번 기록 해석 호출은 1,326,914토큰이다. 정책 계산과 재판정은 모델을 다시 부르지
않고 코드로 실행했다.

작은 15건 말뭉치에 후보 검색을 붙이는 평가는 실행하지 않는다. 질환 범위와 후보가
이미 주어진 상태라 목표 시험을 찾는 문제가 지나치게 쉽다. 후보 검색은 큰 방해 문서와
정답이 있는 TREC 2021·2022 재현 결과로 분리해 유지한다. 실제 전체 추천 평가는 질환
정답을 입력에 주지 않는 환자 질의와 충분한 방해 시험 말뭉치를 만들 때 연다.
