...
상태: 자연어 입력부터 후보 검색·질문·재판정까지 코드 연결 완료, 자연어 정확도 평가 전
정리일: 2026-08-22
...
검사는 공개 원문과 환자 기록의 자연어 구조화 정확도, 잘못된 인용 거부율과 전체 실행
토큰을 외부 모델 소표본에서 측정하는 일이다.
...
- 구조화 입력의 질문 정책 결과와 자연어 전체 실행·임상 성능을 구분한다. 현재
  자연어 전체 연결 코드는 구현됐지만 외부 모델 전체 실행과 임상 성능은 `미측정`으로 표시한다.
...
안전성을 확인한 결과가 아니다. 자연어 전체 실행의 코드 연결은 끝났으며, 현재 이어서
할 일은 외부 모델 소표본에서 구조화 정확도와 같은 출력 형식이 유지되는지 확인하는
것이다. 부담 단계와 승인 경계의 의료 전문가 검토는 실제 환자 적용 전 미해결 조건으로
남긴다.
