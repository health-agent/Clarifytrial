...
상태: 첫 구현·조건 평가·후보 검색·대화형 질문 정책·환자 맞춤 부담 구조 평가 완료
정리일: 2026-08-21
...
질문·확인 요청문을 담당한다. 공개 원문과 환자 기록의 자연어 구조화 정확도를 확인한
뒤 후보 검색부터 이 실행기까지 한 명령으로 연결한다.
...
안전성을 확인한 결과가 아니다. 현재 이어서 할 일은 자연어 전체 실행에서 같은 출력이
유지되는지 확인하는 것이다. 부담 단계와 승인 경계의 의료 전문가 검토는 실제 환자
적용 전 미해결 조건으로 남긴다.
