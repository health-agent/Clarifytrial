...
- 수치, 기간, 명시적 상태처럼 비의료인도 원문으로 확인 가능한 조건 60개 이상
- 새 기본 환자 30명: 질환별 10명
- 환자마다 같은 임상값을 가진 근거 상태 두 가지: 총 60회

새 시험 15건에서 객관적 조건이 60개보다 적으면 조건 수를 맞추기 위해 시험을 더
추가한다. 조건을 골라 빼서 수를 맞추지 않는다. 조건 발견률을 계산할 시험은 객관적으로
정답화할 수 있는 조건을 빠짐없이 기록한다.
...
5. 새 시험·환자 정답을 확정하고 파일을 읽기 전 코드와 지시문 버전을 고정한다.
6. 고정 평가의 원문 정리 결과를 한 번 실행한다. 결과에 맞춰 다시 수정하지 않는다.
7. 개발 환자 12회에서 전체 연결을 검사한다. 질환별 4회이며 오류가 없을 때만 고정
   평가 60회를 연다.
...
예비 정답이다. 의사 정답이나 두 사람 합의 정답으로 부르지 않는다. 조건이 4줄보다
적게 남은 본 시험 6건은 예비 시험과 비교해 교체하거나, 복합 조건 표현을 먼저 추가한
뒤 사용한다. 그다음 구조화된 환자 30명을 만들고 같은 임상값에 근거 상태만 다른
60개 사례로 확장한다.
