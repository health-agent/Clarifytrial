...
| v5 질문 정책 결과 | 남은 질문 횟수 안의 사실 조합을 정확히 계산하는 v3 채택 |
| 환자 맞춤 추가 부담 | 360개 상황·1,800개 정책 평가와 여러 시험 에이전트 흐름 연결 완료 |
| 기본 입력 규격 | 환자 사실·시험 조건·부족 정보·확인 방법을 담은 JSON으로 고정 |
| 자유 형식 입력 연결 | 합성 기록을 JSON으로 정리한 뒤 질문·재판정하는 선택 기능 검사 완료 |
| 새 자연어 평가 예비자료 | 공개 시험 15건·조건 92개, 개발 환자 30명과 새 평가 환자 30명 제작 완료 |
| v5 합성 자연어 예비평가 | 질문 3회 뒤 시험 상태 회복: 질문 없음 42%, 고정 순서 75%, v3 89% |
| 실제 진료기록·임상 성능 | 현행 JSON 입력 범위 밖이며 미측정 |
| 후보 검색과 정적 조건 판단 | 별도 예비실험 완료, v5 성능으로 합치지 않음 |
| 과거 합성 환자 Solar 실행 | 형식 검증 기록으로만 보존 |
| 과거 데모 수치의 v5 기준선 사용 | 제외 |

현재 결과는 AI가 예비 검토한 공개 조건과 규격화된 합성 환자 상태를 사용한 질문 정책
평가다. 자유 형식 기록을 JSON으로 바꾸는 기능은 선택 연결 기능으로 두며, 실제 환자,
의료진 정답과 임상 성능으로 넓혀 해석하지 않는다.
