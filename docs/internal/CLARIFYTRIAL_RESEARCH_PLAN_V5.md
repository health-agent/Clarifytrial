...
| 전수 평균 질문 나무 | 개발 분포 또는 50:50 가정에서 평균 회복이 가장 큰 질문 선택 | 제안 정책 |
| 전수 최악 우선 질문 나무 | 가장 불리한 답의 회복을 먼저 지키고, 동률이면 평균 회복이 큰 질문 선택 | 분포 불확실성 비교 |
| 단일 LLM | 한 모델이 사실·행동 선택과 재판정을 모두 수행 | 단일 구조 기준선 |
| ClarifyTrial v5 | 전수 결정 트리가 사실·경로를 선택하고 LLM은 원문·답변 해석과 문장을 담당 | 제안 방식 |
...
30명 고정 평가에서 전수 정책이 가장 강한 단순 정책보다 시험 상태 회복을 5%p 이상 높이거나,
같은 회복률에서 행동 부담을 20% 이상 줄이지 못하면 복잡한 질문 나무를 주 기여로 삼지 않는다.
개발 분포에서만 좋아지고 50:50 또는 불리한 분포에서 무너지면 분포에 맞춘 결과로 보고 채택하지
않는다.
...
1. 전체 환자 상태, 처음 공개할 사실, 숨은 사실과 행동 결과 자료형 구현
2. 조건과 사실의 연결표에서 전체 정보 정답과 최소 충분 사실 묶음을 계산
3. 제2형 당뇨병·유방암·주요우울장애에서 기본 환자 12명의 작동 확인 자료 작성
4. 모든 사례가 전체 정보 정답, 마스킹, 행동 경로와 재판정 검사를 통과하는지 확인
5. 질문 없음, 모두 확인, 무작위, 영향 범위, 영향 대비 비용 기준선 구현
6. 현재 세 에이전트 흐름을 같은 사실 ID·행동 한도에 연결
7. 외부 모델 없이 12명 전체 흐름을 재생하고 오류를 고침
8. GPT-5.6 Sol `medium`으로 12명 작동 확인 실행
9. 기본 환자 30명, 마스크 60회로 확대해 개발 10명과 고정 평가 20명 분리
10. 후보 결과 회복, 잘못된 제거, 너무 이른 확정, 질문과 비용 비교
11. 주 실험을 고정한 뒤 숫자·기간 조건의 near-miss 보조 분석 추가
12. 가능하면 코디네이터 또는 임상의 소규모 비교
