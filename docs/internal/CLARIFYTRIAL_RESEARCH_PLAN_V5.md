...
| 환자 맞춤 추가 부담 | 선택 입력 기본 규칙·두 결과 형식·360회 비교·채택 기준 설계 완료, 구현 전 |
...
9. 다음 — 사실별 여러 획득 경로, 환자 상황표와 경로별 추가 부담 구현
10. 다음 — 입력 없음·일부 입력의 기본 규칙과 두 결과 형식 구현
11. 다음 — 새 검사·침습 절차·치료 변경의 사람 승인 경계와 보류 검사
12. 다음 — 기존 30명에 부담 조건을 짝으로 붙인 360회 비교 실행
13. 다음 — 공개 원문과 환자 문장을 구조화하는 모델 정확도 평가
14. 다음 — 후보 검색부터 질문·답변·재판정까지 자연어 전체 실행 연결
15. 후속 — 숫자·기간 조건의 near-miss 보조 분석
16. 가능하면 코디네이터 또는 임상의 소규모 비교
