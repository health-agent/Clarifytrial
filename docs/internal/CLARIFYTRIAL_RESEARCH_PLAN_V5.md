...
| 항목 | 확정 값 |
|---|---|
| 작동 확인 자료 | 기본 환자 12명, 환자당 후보 시험 5개, 숨은 사실 3개 |
...
4. 그중 사실 3개를 감추고 처음 보이는 기록을 만든다.
...
`사실 ID`와 `행동`을 출력한다. 숨은 사실이 최대 3개이므로 가능한 공개 순서를 모두
...
| 모두 확인 | 모든 숨은 사실을 가능한 경로로 확인 | 높은 업무량 기준선 |
...
1. 잘못된 후보 제거를 막을 가능성이 있는 사실
2. 더 많은 후보 시험에 공통으로 영향을 주는 사실
3. 더 많은 미해결 조건을 줄이는 사실
4. 같은 영향이면 비용이 낮은 확인 경로
