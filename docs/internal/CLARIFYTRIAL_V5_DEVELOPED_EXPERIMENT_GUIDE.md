...
합친 선택 방법 계산은 2,073,600회다. 현행 논문과 결과 요약은 확인 2·3회의 단계별
무작위·현재 영향 우선·조합 탐색을 주 비교로 사용한다. 확인 1회 실행은 첫 선택의
효과와 재현성을 확인하는 보조 진단으로 보존한다.
...
학습한 결정 순서는 익숙한 정보 분포에서 좋아졌으나 분포를 바꾼 확인 1회 보조
진단에서는 순위가 뒤집혔다. 현재 기본 규칙은 정리 성능이 비슷하고 계산 근거가 그대로
보이며, 미리 정한 답의 분포를 사용하지 않는다.
