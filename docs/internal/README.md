...
| [새 시험 모델 호출 비교](results/independent-new-trial-agent-evaluation-v1/report.md) | 같은 최종 평가자료에서 세 실행 구조의 결과·호출·토큰 비교 |
...
- 별도 평가 환자 30명에서는 추가 확인 세 번으로 실제 참가 가능 후보 47/54개를
  확정하고, 결국 제외될 후보 68/68개를 정리했다.
- 강한 단순 질문 방식과 결과가 같으므로 새 질문 알고리즘의 우월성을 주장하지 않는다.
- 기존 평가와 겹치지 않는 공개 시험 30건을 개발용과 최종 평가용으로 나눴다.
- 최종 평가의 구조화된 판단 75개는 코드만 사용하거나 GPT-5.6 Sol `medium`을 사용한
  세 실행 구조에서 모두 같았다. 모델 호출은 결과를 높이지 않고 1,033,320~1,565,829
  토큰을 사용했다.
- 구조화된 조건은 코드가 계산하고, 뜻을 코드로 정할 수 없는 조건에만 모델을 사용한다.
