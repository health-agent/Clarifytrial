...
- 모델 호출은 진행 관리, 검색·판단, 다음 확인의 세 역할로 나뉜다.
...
- 새 합성 환자 30명에서 세 번 확인한 뒤 최종 답과 같아진 시험은 질문 없음 42%,
  고정 순서 75%, 현재 방식 89%였다.
- 기본 입력은 환자 상태와 시험 조건을 정해진 칸에 넣은 JSON 파일이다. 자유롭게
  작성된 기록을 JSON으로 바꾸는 기능은 선택 사항이다.
- 공개 시험 15개 검색, 후보 조건 판단, 질문, 합성 답변, 재판정과 최종 결과를 한
  터미널에서 보는 실제 모델 실행 명령을 포함했다. 외부 모델 없이 질문 순서만 보는
  축소 화면도 따로 남겼다.
