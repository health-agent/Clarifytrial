...
실험 상태: 공개 시험 589건 검색·조건 202개·합성 환자 50명의 결정론적 전체 평가 완료, 외부 모델 성능 미측정
...
- **한 일:** 불확실하거나 빠진 시험 정보를 연구진에게 확인하고 답을 반영해 선별
  흐름을 다시 실행했다.
- **가져온 부분:** 정보를 얻지 못했을 때 종료하거나 다른 확인으로 넘어가고, 새 답이
  들어오면 관련 판단을 다시 실행하는 비교 근거.
- **공개 자료:** [논문](https://www.sciencedirect.com/org/science/article/pii/S2561326X26005871)
- **적용 범위:** 외부 확인과 재판정 자체를 ClarifyTrial의 새 기여로 주장하지 않는다.
...
- **적용 범위:** 실제 질문 점수는 답 하나로 판단이 달라질 시험 수와, 환자 정보와
  시험 조건이 얼마나 분명하게 연결되는지를 함께 본다. ClarifyTrial은 이 가운데
  판단이 달라질 시험 수를 질문 순서에 반영한다.
...
- ClarifyTrial v5의 결정론적 전체 실행과 외부 모델의 자연어 조건 해석 성능을 구분한다.
  현재는 공개 시험 589건 검색부터 구조화 조건·합성 답을 사용한 재판정까지 측정했다.
