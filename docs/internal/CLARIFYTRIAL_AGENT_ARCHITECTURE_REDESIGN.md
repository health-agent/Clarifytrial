...
상태: 설계·첫 구현·GPT-5.6 Sol 개발 구조 비교 완료
정리일: 2026-08-21

현재 일반 실행기는 다음 확인 에이전트가 사실과 경로를 고른다. 대화형 평가기에는 그 선택을
전수 계산 코드로 바꾼 후보 구조가 따로 구현되어 있다. 30명 고정 평가에서 단순 규칙보다 이득이
확인된 뒤에만 일반 실행기의 선택기를 교체한다.
...
다음 확인은 고정된 질문 순서가 아니라 남은 행동 한도 안의 질문·답변 가지를 코드로
계산해 고른다. 각 가능한 답에서 후보 유지와 현재 확인을 다시 계산한 뒤 다음 순서로 선택한다.

1. 잘못된 후보 제거와 너무 이른 적합 확정이 적은 가지
2. 전체정보 후보 결과를 더 많이 회복하는 가지
3. 위 두 결과가 같으면 행동 수와 환자 부담·비용이 적은 가지

답의 가능성은 평가 환자의 실제 답에서 얻지 않는다. 개발 자료의 분포에서 평균 결과가 가장 좋은
나무와, 어떤 답에서도 가장 덜 손해 보는 나무를 따로 계산한다. 최악의 결과가 같으면 평균 결과로
순서를 정해 지나치게 방어적인 선택을 줄인다. 50:50 가정과 불리한 답에서도 선택이 유지되는지
확인한다. DQueST식 영향 범위와 Fink식 비용 점수는 비교군으로 남긴다.

통과·위반을 사실마다 독립적으로 만든 경우의 수는 코드 진단에만 쓴다. 주 실험은 개발 자료에서
미리 정한, 임상적으로 함께 나타날 수 있는 환자 전체 상태 묶음을 사용한다.
...
이 절은 평가 중인 후보 구조다. 현재 일반 실행기의 다음 확인 에이전트는 그대로 보존하며,
주 실험의 채택 기준을 통과하기 전에는 후보 구조를 최종 구조라고 부르지 않는다.
...
| 다음 확인 | 전수 계산 코드 + 문장 생성 LLM | 결과가 아직 열려 있을 때 | 사실 ID·확인 경로는 코드, 질문·요청문만 LLM |
