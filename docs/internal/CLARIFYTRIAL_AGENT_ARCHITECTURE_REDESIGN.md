...
ClarifyTrial의 기본 구조는 **코드 진행 관리와 필요할 때만 부르는 모델 역할**이다. 모델
역할은 최대 다섯 개지만, 이미 구조화된 JSON만 입력하면 외부 모델 호출은 0회다. 자유
문장 입력이나 코드로 뜻을 정할 수 없는 조건이 있을 때만 필요한 역할을 부른다.

| 모델 역할 | 호출 시점 | 맡은 일 |
|---|---|---|
| 환자 기록 정리 | 자유 문장 환자 기록을 받을 때 1회 | 검색 질환, 환자 사실과 근거 문구를 JSON으로 정리 |
| 시험 조건 정리 | 새 자유 문장 시험 원문을 받을 때 시험별 1회 | 선정·제외 조건과 필요한 환자 사실을 JSON으로 정리 |
| 조건 판단 | 구조화 규칙으로 뜻을 정할 수 없는 조건이 있을 때 | 해당 조건의 상태, 근거와 부족 정보를 판단 |
| 질문 문장 작성 | 선택한 확인 내용을 자연스러운 문장으로 보여 줄 때 | 코드가 고른 사실과 방법을 바꾸지 않고 요청문 작성 |
| 선택적 검토 | 실제 근거 충돌이나 구조화하지 못한 중요 조건이 남았을 때 | 승인, 관련 조건 재판정 또는 추가 확인 제안 |

진행 관리, 검색, 수치·날짜 계산, 다음 정보 선택, 환자 제한 적용과 종료 결정은 코드가
맡는다. 같은 언어모델을 여러 역할에 쓸 수 있지만 역할별 지시문, 입력, 대화 기록과
출력 형식은 분리한다.

준비 역할은 입력을 공통 JSON으로 옮길 뿐 대화를 이어 가거나 다음 행동을 고르지 않는다.
근거가 문서 어느 부분에 있는지는 코드가 찾는다. 띄어쓰기와 줄바꿈은 비교하지 않고,
중요 수치·날짜·비교 방향과 출처가 맞는지 확인한다.

구조화된 최종 평가에서는 코드만 사용한 방식과 모델 역할을 부른 두 방식이 모두
75/75개로 같았다. 외부 모델 호출은 각각 0회, 65회, 105회였다. 이 결과에 따라
구조화된 JSON의 기본값을 코드 실행으로 정했다.
...
| 보내는 쪽 | 받는 쪽 | 전달 내용 |
|---|---|---|
| 코드 진행 관리 | 조건 판단 | 현재 환자 사실, 후보 시험, 관련 조건과 이전 판단 |
| 조건 판단 | 코드 진행 관리 | 조건별 임상 상태, 현재 자료의 충분성, 양쪽 원문 ID와 부족 사실 ID |
...
- 수치, 단위와 비교 방법
- 요구 기간과 판단 기준일
- 모두 충족, 하나 이상 충족, 일정 개수 이상 충족, 부정과 예외
- 환자 답변으로 확인 가능한지 여부
...
코드는 조건을 나무 구조로 저장해 `모두`, `하나 이상`, `일정 개수 이상`을 계산한다.
공개 평가자료에는 원문에서 개수 관계가 분명한 3개 시험의 4묶음을 연결했다. 어느
하위 조건이 충족·위반·미확인인지도 함께 남는다. 한 경로만 충족해도 되는 조건에서
이미 한 경로가 충족되면, 다른 경로에 남은 정보는 더 묻지 않는다. 예외와 시험군별
적용 범위를 안전하게 구조화하지 못한 문장은 필수 조건 목록으로 평평하게 만들지 않고
실행 대상에서 제외한다.
이 단계는 TrialMatchAI와 EXACT의 조건 구조화를 참고한다.
...
제외한다. 공개 시험 기반 전체 실행에서는 질환 이름 변형과 BM25로 상위 200건을 찾은
뒤 미리 정한 정밀 평가 시험만 조건 판정에 넘겼다. 서로 다른 시험은 50건이고,
30명에게 시험 5건씩 반복해 만든 150개 연결이 모두 검색됐다. 검색된 다른 시험은
조건 판정하지 않았으므로 이 결과는 검색 연결 검사이며 검색 일반화 성능이 아니다.
검색기는 공통 계약 뒤에 있어 MedCPT 결합 검색으로 바꿔도
뒤쪽 조건 판단과 질문 코드는 바뀌지 않는다.
...
실행 기록에 남긴다. 구조화된 조건에서 모델이 남긴 `정보가 없음`이나 `설명이 약함`
표시는 같은 검사를 되풀이하므로 검토를 부르지 않는다. 별도 검토 에이전트는 다음 중
하나에 해당할 때만 부른다.

- 코드가 계산할 수 없는 조건의 뜻이 하나로 정리되지 않음
- 코드가 실제로 서로 충돌하는 환자 근거를 발견함
- 조건 원문이나 환자 근거 ID가 현재 입력 범위를 벗어남
...
가능한 환자 답의 확률을 가정하는 평균 우선과 최악 우선 방식은 별도 평가에서 이득이
유지되지 않아 사용하지 않는다. 현행 방식은 답의 확률을 추측하지 않고 남은 확인 횟수
안에서 판단을 끝낼 수 있는 정보 조합을 계산한다. 강한 비교 방식은 남은 횟수 전체를
보지 않고 지금 가장 많은 미완료 시험에 연결된 정보를 하나씩 고른다.

공개 시험 조건 기반 별도 평가 환자 30명에서 두 방식은 최종 상태 일치, 실제 후보 확정과
제외 후보 정리가 모두 같았다. 전체 조합 계산은 지원 기능으로 유지하지만 큰 성능 향상
근거로 사용하지 않는다. DQueST식 영향 범위와 Fink식 비용 고려는 두 방식의 공통 원칙이다.

#### 확인 방법과 환자 부담

이동·비용·시간 제한을 반영하는 모듈은 `LOOKUP_RECORD`, `ASK_PATIENT`,
`REQUEST_VERIFICATION`이라는 상위 행동을 유지하되, 각 행동 아래의 구체적인 확인
방법을 별도로 기록한다.
...
| 조건 판단 | 구조화 규칙 또는 선택 모델 역할 | 처음과 관련 환자 사실이 바뀔 때 | 바뀐 조건의 상태와 근거 번호 |
...
| 결과 보고 | 공통 코드 | 종료 시 | 근거, 남은 정보, 질문 전후 변화와 실행 요약 |
...
```text
표준 JSON 환자 상태 읽기
→ 기본 실행은 저장된 공개 시험 50건에서 후보 5건 검색
→ 공개 시험 모음을 지정하면 모집 중·모집 예정 시험 589건에서 후보 5건 검색
→ 후보 시험의 저장된 조건 불러오기
→ 코드가 다음 단계를 정하고 조건 판단 역할 호출
→ 코드가 확인할 정보와 경로 선택
...
`run-full-ui`의 기본 예제는 공개 시험 50건 중 같은 질환의 후보 5건을 골라 연결을 보여
준다. 외부 모델 없이도 실행할 수 있고, 실제 모델을 선택했을 때만 실행 확인을 요구한다.
`--broad-corpus`를 지정하면 전체 평가와 같은 589건 검색을 화면 앞단에 붙인다. 수만 건
검색의 별도 성능은 TREC 평가 명령에서 확인한다.
...
각 끝 조건은 근거·부족 정보·수치·기간 검사와 연결한다. 상위 조건식은 끝 조건의
`충족`, `위반`, `미확인`, `충돌` 상태를 묶고 어느 가지 때문에 후보 유지·확정·제외가
됐는지 남긴다. 공개 평가자료에는 `둘 중 하나`, `셋 중 둘`, `넷 중 둘`을 명시한
3개 시험의 4묶음이 들어 있다.

시험군별 적용 범위와 예외는 아직 임의로 추측하지 않는다. 입력에 명시적 조건 나무가
없으면 복잡한 문장을 필수 조건 목록으로 바꾸지 않고 구조화 대상에서 제외한다.

## 6. 모델과 실행 조건

| 입력과 목적 | 기본 실행 |
|---|---|
| 구조화된 환자·조건 JSON | 외부 모델 없이 코드 계산 |
| 자유 문장 조건의 뜻 정리 | 설정한 조건 판단 모델 호출 |
| 질문 문장을 자연스럽게 작성 | 설정한 질문 문장 모델 호출 |
| 실제 근거 충돌 확인 | 선택적 검토 모델 최대 1회 |

구조 비교에는 GPT-5.6 Sol `medium`을 사용했다. 구조화 규칙만 사용한 실행과 모델을
65회·105회 부른 실행의 최종 결과가 모두 75/75개로 같아 구조화 JSON의 기본값을
외부 모델 없는 실행으로 정했다.

모델 제공자는 공통 어댑터로 바꿀 수 있으며 진행 코드와 평가기는 그대로 둔다. 사례에
따라 모델을 자동으로 바꾸지 않는다. 외부 모델을 사용한 실행에는 모델 이름, 추론 설정,
역할별 호출·토큰, 시간과 실패를 실행 기록에 남긴다. 상세 검색과 평가 조건은
[시험 검색과 평가 구현](CLARIFYTRIAL_RAG_EVALUATION_IMPLEMENTATION_PLAN.md)에 있다.

## 7. 호출 규칙과 출력 형식

시험 조건과 환자 상태표는 공통으로 미리 계산해 구조 비교에 제공한다. 외부 모델이
필요한 입력에서 한 번의 진행 주기 호출 수는 다음처럼 계산한다.

```text
구조화하지 못한 조건이 있으면 조건 판단 1회
+ 질문 문장을 모델로 작성하면 다음 확인 1회
+ 실제 근거 충돌이 있으면 선택적 검토 1회
+ 형식 오류가 있으면 수정 1회
```

구조화된 JSON 기본 실행은 위 역할을 부르지 않는다. 모델이 필요하면 처음 판단하거나
새 정보가 들어온 뒤 바뀐 시험과 조건만 한 요청으로 묶는다. 조건은 최대 40개씩 나누고,
시험 원문은 8,000자보다 길면 줄 경계에서 나눈다. 응답에서 일부 조건만 빠지면 성공한
조건은 보존하고 빠진 조건만 다시 요청한다. 비교가 필요하면
...
자료형과 호출 제한은 이 문서 7절, 검색과 평가 범위는
[시험 검색과 평가 구현](CLARIFYTRIAL_RAG_EVALUATION_IMPLEMENTATION_PLAN.md)에 둔다.
...
현재 공개 시험 기반 통합 점검에서는 합성 환자 30명에게 추가 확인을 세 번 허용했다.
남은 확인 횟수 전체를 계산한 방법은 150개 시험 판단 중 143개를 정해진 최종 상태까지
정리했고, 입력 순서 방식은 141개였다. 환자별로는 1명에서 좋고 29명에서 같았다. 지금
가장 많은 시험에 연결된 정보를 고르는 단순한 방법도 143개로 같았다. 전체 조합 계산은
일반 실행기에 남기되 성능 우월성은 주장하지 않는다.
...
10. **ClarifyTrial v5**
   - 코드 진행 관리, 조건 판단, 다음 확인, 선택적 검토 역할
...
구조화된 조건에서 다음 세 방식을 같은 최종 평가자료로 비교했다.

1. 코드가 수치·날짜·참거짓 조건을 직접 계산
2. 조건 판단에만 GPT-5.6 Sol `medium` 사용
3. 코드가 순서를 통제하고 조건 판단과 질문 문장 작성에 같은 모델 사용

기존 평가와 겹치지 않는 공개 시험 15건, 합성 환자 25명과 환자–시험 판단 75개를
사용했다. 기대 결과는 현재 판정 코드와 별도로 만든 계산표에 먼저 고정했다.

| 실행 구조 | 최종 상태 일치 | 실제 후보 확정 | 제외 후보 정리 | 외부 호출 | 토큰 |
|---|---:|---:|---:|---:|---:|
| 구조화 규칙만 사용 | 75/75 | 36/36 | 23/23 | 0회 | 0 |
| 조건 판단 모델만 사용 | 75/75 | 36/36 | 23/23 | 65회 | 1,033,320 |
| 조건 판단과 질문 문장 역할 호출 | 75/75 | 36/36 | 23/23 | 105회 | 1,565,829 |

세 방식에서 잘못 제외, 성급한 확정과 실행 실패는 모두 0개였다. 구조화된 조건에서는
모델 호출을 늘려도 최종 결과가 좋아지지 않았다. 따라서 기본 실행은 코드를 사용하고,
조건의 뜻을 코드로 정할 수 없을 때만 조건 판단 모델을 부른다. 별도 검토는 단순히
정보가 부족하다는 이유로 부르지 않고, 근거가 서로 충돌하거나 구조화하지 못한 조건이
남았을 때만 부른다.

자세한 실행값은 [모델 호출 방식 비교](results/independent-new-trial-agent-evaluation-v1/report.md)에 있다.

## 11. 현재 주장 범위

현재 구현에는 공통 JSON, 모집 상태를 거른 시험 검색, 구조화된 수치·날짜·출처 검사,
복합 조건 계산, 합성 답변 환경, 환자 부담 규칙, 중단·재개와 보고서 생성이 들어 있다.

공개 시험 기반 통합 점검에서는 합성 환자 30명·시험 판단 150개를 검색부터 질문 뒤
재판정까지 연결했다. 추가 확인 세 번으로 실제 후보 47/54개를 확정하고 제외될 후보
68/68개를 정리했다. 이 값은 같은 구조화 규칙으로 만든 기대 상태를 사용한 소프트웨어
점검이다.

새 시험 최종 평가에서는 기존 시험과 겹치지 않는 공개 시험 15건·합성 환자 25명·판단
75개를 별도 계산표와 비교했다. 세 모델 호출 구조의 결과가 모두 같았으므로 여러
에이전트가 정확도를 높였다고 주장하지 않는다.

강한 단순 정보 선택 방법과 ClarifyTrial의 질문 순서는 30명 모두 같은 결과였다.
복잡한 질문 계산의 우월성, TrialGPT보다 높은 전체 추천 정확도와 실제 임상 업무 효과를
주장하지 않는다.

ClarifyTrial의 중심은 후보 유지와 현재 확인을 분리하고, 보존한 후보 중 실제로 확정한
수와 결국 제외한 수, 그 과정의 질문·검사·방문을 같은 실행에서 평가하는 데 있다.
