...
뒤 정밀 평가 대상으로 정한 시험만 조건 판정에 넘겼다. 30명에게 연결한 평가 대상
150건은 모두 검색됐지만, 이 50개 시험은 같은 시험 모음에서 골랐으므로 검색 일반화
성능으로 해석하지 않는다. 검색기는 공통 계약 뒤에 있어 MedCPT 결합 검색으로 바꿔도
...
`모두`·`하나 이상`·`일정 개수 이상` 조건 계산, 역할별 모델 호출, 합성 답변 환경,
환자 부담 규칙, 중단·재개와 보고서 생성이 들어 있다.

팀 공개 시험 1,931건 가운데 모집 중·모집 예정 589건을 검색하고, 10개 질환·공개 시험
50건의 구조화 조건 202개와 합성 환자 50명을 연결했다. 별도 평가 환자 30명에서는
589건 검색부터 질문 뒤 재판정까지 같은 사례로 실행했다. 현재 확인이 끝난 시험만
보여 주면 보이지 않을 실제 참가 가능 후보 54개를 모두 보존했고, 추가 확인 세 번으로
47개를 확정했다. 결국 제외될 후보 60개는 모두 정리했다.

이 결과는 외부 언어모델 없이 정해진 조건 계산과 저장된 합성 답을 사용한 실행이다.
프로그램 규칙과 자료 연결을 검증한 수치이며 자연어 조건 해석의 최종 성능은 아니다.
외부 모델 30명 실행, 호출·토큰과 시간 측정은 남아 있다.

강한 단순 정보 선택 방법과 남은 횟수 전체를 계산한 방법은 30명 모두 같은 결과였다.
따라서 복잡한 질문 계산의 우월성, TrialMatchAI·CLEAR-MATCH·EXACT보다 높은 정확도와
실제 임상 업무 효과를 주장하지 않는다.

ClarifyTrial의 중심은 같은 환자 사실에서 후보를 남기는 판단과 현재 자료로 조건을
확인하는 판단을 분리하고, 보존한 후보 중 실제로 확정한 수와 결국 제외한 수, 그 과정의
질문·검사·방문을 같은 실행에서 평가하는 데 있다.
