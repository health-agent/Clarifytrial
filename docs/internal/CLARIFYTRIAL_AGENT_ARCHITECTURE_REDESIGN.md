...
끝 조건의 결과를 묶고, 어느 가지 때문에 후보 유지·확정·제외가 됐는지 남긴다. 후보
검색 순위는 같은 결과 묶음 안에서 보존한다. 서로 다른 종류의 조건에 억지로 하나의
거리 점수를 만들어 추천 순서를 바꾸지 않는다. 실제 원문에서 조건식과 시험군 정답을
만들 방법이 정해지기 전까지는 연구 구상으로만 둔다.
