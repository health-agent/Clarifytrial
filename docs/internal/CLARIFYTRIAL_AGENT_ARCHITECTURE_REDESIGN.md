...
상태: 합성 자연어 기록 해석과 새 질문 정책 평가 완료, 사람 검토·실제 기록 평가는 전
정리일: 2026-08-22

현재 일반 환자 실행기는 여러 후보 시험의 미해결 조건을 함께 보고, 남은 질문 횟수로
...
실행 시작 전에는 환자 기록과 후보 시험 원문을 구조화하는 두 준비 호출이 따로 있다.
...
자연어 입력을 받을 때는 반복 흐름에 들어가기 전에 다음 두 역할을 호출한다.
