...
질환 10개·합성 시험 50개·합성 환자 50명의 성숙도 점검 자료에서는 현재 확인이 끝난
시험만 보여 줄 때 보이지 않을 실제 참가 가능 후보, 추가 확인 후보로 보존한 수,
질문 뒤 실제 후보로 확정한 수와 결국 제외되는 시험을 따로 계산한다. 같은 실행에서
새 검사·추가 방문과 답을 얻지 못한 확인도 저장한다. 이 자료는 합성 시험 조건이므로
전체 에이전트 연결과 보고서가 준비됐는지 확인하는 용도다.
