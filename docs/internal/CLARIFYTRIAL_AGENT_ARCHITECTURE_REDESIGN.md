...
상태: 기본 구조·공개 조건 질문 정책 평가 완료, 환자 맞춤 부담 상세 설계 완료·구현 전
정리일: 2026-08-21
...
다음 구현에서는 `LOOKUP_RECORD`, `ASK_PATIENT`, `REQUEST_VERIFICATION`이라는 상위
행동은 유지하되, 각 행동 아래의 구체적인 획득 방법을 별도로 기록한다.
...
방법과 환자 선호를 구조화할 수 있지만, 코드가 허용 경로와 사람 승인 여부를 검사한다.
...
| 다음 확인 | 환자 맞춤 단순 규칙 + 문장 생성 LLM | 결과가 아직 열려 있을 때 | 사실 ID·확인 경로·추가 부담은 코드, 질문·요청문만 LLM |
...
다음 확인 에이전트는 숨은 답을 추측하지 않는다. 현재 대화형 평가기의 단순 규칙은
영향을 받는 후보 수, 미해결 조건 수, 1·2·3 간이 경로 비용 순으로 사실 하나를
고른다. 이 값은 실제 환자 부담이 아니다. 다음 구현은 사용할 수 없는 경로와 환자
한도 위반을 먼저 제거하고, 기존 자료를 새 검사보다 우선한 뒤 환자가 밝힌 선택
방식에 따라 대기와 추가 부담을 비교한다. 같은 우선순위에서는 사실 ID로 순서를 고정한다.
...
9. **환자 맞춤 추가 부담**
   - 여러 획득 경로, 기존 자료 우선, 환자 상황표와 사람 승인 경계
   - 고정 비용·부담 최소·환자 맞춤 방식의 360회 짝 평가
10. **ClarifyTrial v5**
