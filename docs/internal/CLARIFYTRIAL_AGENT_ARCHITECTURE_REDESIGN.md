...
상태: 규격화된 JSON 전체 흐름과 새 질문 정책 평가 완료, 자유 형식 입력은 선택 연결 기능
정리일: 2026-08-22

현행 연구의 기본 입력은 환자 사실, 시험 조건, 부족한 정보와 확인 가능한 방법을 담은
규격화된 JSON이다. 일반 환자 실행기는 여러 후보 시험의 미해결 조건을 함께 보고, 남은 질문 횟수로
끝낼 수 있는 시험이 가장 많은 사실 묶음을 전부 계산한다. 사실을 고른 뒤에는 환자
부담과 승인 규칙으로 확인 경로를 고른다. 과거의 답 확률별 질문 나무는 채택 기준을
통과하지 못해 비교용으로만 남겼다. LLM은 조건
판단과 질문·확인 요청문을 작성하지만, 코드가 고른 사실·경로·관련 조건을 바꿀 수 없다.
자유 형식 기록을 받을 때만 실행 전에 환자 기록과 후보 시험 원문을 JSON으로 옮기는
두 준비 호출을 사용한다.
코드는 근거 문구를 원문에서 다시 찾고 판단을 바꾸는 수치·단위·날짜·비교 방향과
검사 유효기간이 맞는지 확인한다. 띄어쓰기와 줄바꿈은 판정하지 않는다. 후보 검색은
모델이 다시 붙인 질환명이 아니라 환자 기록에서 코드가 찾은 실제 문구를 사용한다.
...
두 역할은 대화 맥락을 유지하거나 다음 행동을 고르는 에이전트가 아니다. 입력을
공통 자료형으로 옮기는 준비 호출이다. 근거 위치는 모델이 맞히는 값이 아니라 코드가
계산한다. 근거 문구가 없거나 핵심값이 원문과 다르면 실행을 중단한다. 원문에서 뽑은
참가 조건은 모두 필수 확인 대상으로 두며, 모델이 선택 사항으로 바꿀 수 없다.
...
공통 부족 사실 병합, 한 번의 자료 확인과 두 시험 재판정까지 검사했다. Sol `medium`
