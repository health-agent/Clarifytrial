...
상태: 자연어 입력 준비·후보 검색·여러 시험 재판정 연결 완료, 자연어 정확도 평가 전
정리일: 2026-08-22

현재 일반 환자 실행기는 여러 후보 시험의 미해결 조건을 함께 보고, 단순 동적 규칙과
환자 부담 규칙으로 다음 사실과 확인 경로를 고른다. 공개 조건 평가에서 복잡한 전수
계산이 채택 기준을 통과하지 못했으므로 일반 실행기에는 넣지 않았다. LLM은 조건
판단과 질문·확인 요청문을 작성하지만, 코드가 고른 사실·경로·관련 조건을 바꿀 수 없다.
실행 시작 전에는 환자 기록과 후보 시험 원문을 구조화하는 두 준비 호출이 따로 있다.
각 출력은 정확한 원문 인용과 문자 위치가 맞아야 다음 단계로 넘어간다.
...
| 준비 역할 | 호출 횟수 | 맡은 일 |
|---|---:|---|
| 환자 기록 정리 | 환자 기록당 1회 | 검색 질환과 환자 사실을 정확한 원문 위치와 함께 추출 |
| 시험 조건 정리 | 검색된 시험당 1회 | 선정·제외 조건, 부족 사실과 정확한 원문 위치를 추출 |

두 역할은 대화 맥락을 유지하거나 다음 행동을 고르는 에이전트가 아니다. 입력을
공통 자료형으로 옮기는 준비 호출이며, 인용 위치가 틀리면 코드가 실행을 중단한다.
...
다음 확인 에이전트는 숨은 답을 추측하지 않는다. 기존 대화형 평가기의 단순 규칙은
영향을 받는 후보 수, 미해결 조건 수, 1·2·3 간이 경로 비용 순으로 사실 하나를
고른다. 이 값은 실제 환자 부담이 아니다. 환자 맞춤 모듈은 사용할 수 없는 경로와
...
공통 부족 사실 병합, 한 번의 자료 확인과 두 시험 재판정까지 검사했다. 외부 모델의
자연어 구조화 정확도와 이 전체 흐름의 추천 성능은 아직 측정하지 않았다.
