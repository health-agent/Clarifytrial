...
`run-full-ui`는 위 실행 단위를 한 화면에서 순서대로 보여 준다. 연구의 기본 입력인
표준 JSON 환자 상태와 저장된 시험 조건을 사용한다. 자유롭게 작성된 기록을 JSON으로
바꾸는 준비 호출은 선택 기능이므로 이 기본 화면에 다시 넣지 않는다.
...
이 화면의 검색은 저장된 시험 15개 안에서 연결을 보여 주는 기능이다. 수만 건을
대상으로 한 TrialGPT식 검색 성능은 TREC 평가 명령에서 별도로 확인한다.
