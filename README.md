...
넣은 JSON 파일이다. 자유롭게 작성된 진료기록을 이 형식으로 바꾸는 기능도 있지만,
그 변환 성능은 현재 연구의 평가 대상에 포함하지 않는다.
...
TrialGPT의 공개 답은 전문가 정답과 88.8% 일치했다. 이 부분에서는 TrialGPT보다
낮았다. 한 번 더 검토하거나 인터넷을 검색하게 해도 맞힌 수가 늘지 않아 해당
기능은 사용하지 않기로 했다.
