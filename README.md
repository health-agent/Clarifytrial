...
| 오류 방향 | 차단 기준 누락률, 잘못된 참여 불가 판정률, 자동 확정 보류 비율과 보류 사례 오류율 | 환자-임상시험 쌍 |
...
| 다음 질문과 정보 조합 선택 | [Learning to Ask Medical Questions](https://proceedings.mlr.press/v126/shaham20a.html), [ACO](https://arxiv.org/abs/2302.13960), [ProMed](https://aclanthology.org/2026.acl-long.1500/) | 답에 따라 다음 질문을 고를 수 있고, 한 정보씩만 보는 방식은 함께 얻을 때 유용한 정보 조합을 놓칠 수 있으며, 질문의 정보 가치를 학습할 수 있음 | 한 단계 방식과 최소 결정 정보 집합 방식을 분리해 비교하고 질문 수가 아니라 판정 결정 가능 상태 도달 비용을 평가 |
| 불완전 정보에서 판정 가능 여부 | [ClinDet-Bench](https://aclanthology.org/2026.acl-industry.47/) | 빠진 정보의 가능한 모든 값에서도 결론이 같은지 확인해야 너무 이른 확정과 불필요한 판단 보류를 구분할 수 있음 | 남은 정보의 모든 유효한 값 구간에서 최종 판정이 같은지를 종료 조건으로 사용 |
...
### 12.11 ACO, ProMed, ClinDet-Bench: 정보 조합과 판정 결정 가능성

**ACO가 지적한 문제.** 활성 정보 획득 연구인 ACO는 다음 정보 하나의 즉시 효과만
보는 방식이 여러 정보를 함께 얻을 때 생기는 가치를 놓칠 수 있다고 지적했다.
ClarifyTrial은 복잡한 학습 정책을 그대로 도입하지 않고, 참여 기준 논리 구조에서
최소 결정 정보 집합을 계산해 이 문제를 재현 가능한 규칙으로 시험한다.

**ProMed가 보여준 방향.** ProMed는 질문이 새로 제공하는 정보와 문맥상 중요도를
Shapley 정보 이득으로 계산하고, 몬테카를로 트리 탐색과 강화학습으로 능동적 의료
질문 정책을 학습했다. 임상시험 매칭 연구는 아니지만 단순 질문 수보다 의사결정에
기여한 정보량을 봐야 한다는 근거가 된다. ClarifyTrial의 첫 구현은 학습 정책 대신
논리 기반 정책을 사용하고, 후속 연구에서 같은 평가 자료에 학습 정책을 비교할 수 있다.

**ClinDet-Bench가 보여준 방향.** 이 연구는 빠진 정보의 모든 가능한 값에서도 결론이
같을 때만 현재 정보로 판정 가능하다고 본다. 언어모델이 완전한 정보에서는 잘
판단해도 불완전한 정보에서 너무 일찍 확정하거나 불필요하게 판단을 보류할 수 있음을
보였다. ClarifyTrial은 이 정의를 종료 조건으로 사용하고, 여러 정보를 함께 알아야
하는 사례에서 너무 일찍 끝낸 비율과 불필요하게 계속한 비율을 따로 측정한다.
...
5. **관계망 오류에서 최종 결과까지의 추적:** 잘못된 통합·불필요한 분리·연결 누락이
   ...
   차단 기준 누락과 잘못된 참여 불가 판정도 따로 제시한다.
