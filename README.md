...
시험 조건 정리본은 기본적으로 `runs\trial-protocol-cache`에 저장한다. 다음 환자에게
같은 시험이 다시 나오면 모델이 원문을 다시 읽지 않고 이 저장본을 쓴다. 시험 원문,
조건 정리 지시문 또는 모델 설정이 바뀌면 자동으로 새로 정리한다. 다른 위치를 쓰려면
`--trial-protocol-cache C:\path\to\cache`를 붙인다. 화면과 결과 파일에는 저장본을
몇 개 재사용했고 몇 개를 새로 정리했는지 표시한다. 저장된 내용이 달라졌거나 손상된
경우에도 사용하지 않고 다시 정리한다.
