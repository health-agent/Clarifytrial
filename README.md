...
연구의 기본 입력은 환자 사실, 시험 조건, 부족한 정보와 확인 가능한 방법을 담은
규격화된 JSON이다. 자유 형식의 진료기록을 JSON으로 바꾸는 연결 기능도 있지만,
자연어 해석 성능은 현재 연구의 평가 대상에 포함하지 않는다.

## 한 사례가 처리되는 순서

```text
규격화된 환자 상태와 임상시험 조건 입력
→ 관련 시험 검색
→ 조건별 환자 상태와 근거 확인
→ 후보 유지와 현재 확인을 따로 판단
...
남겼다는 뜻이다. 실제 환자의 비용 절감이나 편익으로 해석하지 않는다.

## 현재 해석 범위

구현된 범위는 규격화된 JSON 입력, 후보 검색, 조건 판단, 여러 부족 정보의 순서 선택,
환자 부담에 맞는 확인 방법, 새 정보 뒤 재판정과 두 결과 목록까지다. 자유 형식 기록을
JSON으로 바꾸는 선택 연결 기능도 Sol `medium` 합성 사례에서 끝까지 실행했다.
