...
후보 검색과 자연어 조건 구조화도 환자 단위 실행과 한 명령으로 연결했다. Sol
`medium` 합성 사례 한 건은 끝까지 실행했지만, 외부 모델이 여러 자연어 원문을 얼마나
정확히 구조화하는지는 아직 측정하지 않았다.
...
원문을 저장한다. 후보 검색에도 모델이 다시 붙인 질환명 대신 환자 기록의 실제 문구를
쓴다. 원문에서 뽑은 참가 조건은 모델이 선택 사항으로 바꿀 수 없다.
...
| 14 | 자연어 기록·시험 원문에서 전체 흐름 실행 | 합성 검사와 Sol 한 사례 연결 완료, 공개 원문 구조화 정확도 평가는 아직 전 |
