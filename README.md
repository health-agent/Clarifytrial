...
| 넓은 합성 성숙도 점검 | 최종 평가 지표와 전체 실행 안정성 | 질환 10개·시험 50개·환자 50명 |
...
기존 세 질환에만 맞는 프로그램인지 확인하기 위해 질환 10개·합성 시험 50개·합성
환자 50명을 별도로 만들었다. 환자마다 정보 한 개·두 개·세 개 또는 다섯 개를 가렸고,
완전한 정보에서는 참가 가능 154개와 제외 96개가 모두 나오도록 구성했다.

개발에 사용하지 않은 환자 30명에서 현재 확인이 끝난 시험만 보여 주면 보이지 않을
실제 참가 가능 후보가 65개였다. ClarifyTrial은 이 65개를 모두 추가 확인 후보로
보존했고, 최대 세 번 확인한 뒤 52개를 실제 후보로 확정했다. 처음에는 보존했지만
완전한 정보에서 제외되는 시험 62개 중 57개도 질문 뒤 제외로 정리했다.

이 실행에서 잘못 제외하거나 정보가 부족한데 확정한 시험은 없었다. 답 하나를 얻지
못하게 했을 때 같은 정보를 반복한 횟수도 0회였다. 프로그램 구조는 실제 공개 시험
조건을 넣어 최종 평가를 시작할 수준이지만, 이 값은 합성 시험 조건과 외부 모델 없는
실행 결과이므로 최종 성능 수치에는 사용하지 않는다.

새 검사와 추가 방문을 허용한 경우에는 실제 후보 52/65개를 확정하며 새 검사와 방문을
각각 4회 사용했다. 같은 환자가 새 검사와 방문을 허용하지 않은 경우에는 그런 행동을
0회 사용하고 실제 후보 45/65개를 확정했다. 높은 회복률과 환자의 선택을 함께 표시한다.
...
- 후보로 보존한 시험, 질문 뒤 실제 후보로 확정한 시험과 결국 제외된 시험의 분리 집계
- 최종 성능평가 준비 상태 자동 점검
...
3. 최종 평가 전 질환 10개·합성 환자 50명의 성숙도 점검은 완료했다.
4. 선택한 공개 시험 50건에서 객관적으로 계산할 조건을 정리한다.
5. 그 실제 조건에 맞는 최종 합성 환자를 만든다.
6. 1,931개 시험 전체에 정답을 붙이지 않고, 검색된 시험 가운데 정밀 평가 대상으로
   고른 시험에만 조건별 정답과 질문 뒤 변화를 만든다.
7. 모집 가능한 589건 검색부터 질문 뒤 순위 변경까지 같은 사례로 실행한다.
8. 마지막에 사용할 외부 모델로 최종 통계와 모델 사용량을 다시 낸다.
...
전체 실행 뒤 `audit-final-evaluation-readiness`에 시험·환자·실행 결과를 넣으면 질환과
정보 부족 범위, 후보 회복 집계, 오류·반복, 실제 공개 조건·넓은 검색·외부 모델 사용
여부를 나눠 보여 준다.
